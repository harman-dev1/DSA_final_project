...
#variables for storing data from UI
first_name = ""
last_name = ""
email = ""
email_password = 0
...
"background-color: rgb(255, 255, 255);")
    ...
    def display_message(self,current_dialog):
       while True:
           ...
           # Check email validity using your 'check_email' function
           if not check_email(entered_email):
               ...
               continue  # Continue to recheck the email

           # Check password length
           if len(str(abs(entered_password))) > 10:
               # Display message box for password length exceeded
               msg_box = QtWidgets.QMessageBox()
               msg_box.setIcon(QtWidgets.QMessageBox.Warning)
               ...
               msg_box.exec_()
               break  # Exit the function if password length is exceeded
           else:
               # All fields are filled and password length is within limits, proceed to store information
               msg_box = QtWidgets.QMessageBox()
               ...
               # Convert to an integer if the string is not empty
               if self.email_password != '':
                   int_password = int(self.email_password)
                   print(first_name)   
               ht_instance.insert(entered_first_name, entered_last_name, entered_email, entered_password)
               ht_instance.save_to_file("SignUp.txt")
